...
// ========== IAT Reconstruction (delegated to Scylla DLL) ==========
    ...
    std::optional<uint32_t> oepRva)
{
    ...
    auto& scylla = ScyllaWrapper::Instance();
    if (!scylla.IsAvailable()) {
        result.error = "Scylla DLL not available: " + scylla.LoadError();
        Logger::Error("[fix_imports] {}", result.error);
        return result;
    }

    // Optionally set OEP in PE header before IAT fix
    if (oepRva.has_value()) {
        Logger::Info("[fix_imports] Setting OEP RVA to 0x{:X} in {}",
                     oepRva.value(), filePath);
        try {
            auto fsPath = ToFilesystemPath(filePath);
            std::fstream peFile(fsPath, std::ios::binary | std::ios::in | std::ios::out);
            if (peFile) {
                ...
                if (dosHeader.e_magic == IMAGE_DOS_SIGNATURE) {
                    ...
                    if (ntHeaders.Signature == IMAGE_NT_SIGNATURE) {
                        ...
                        peFile.seekp(dosHeader.e_lfanew, std::ios::beg);
                        peFile.write(reinterpret_cast<const char*>(&ntHeaders), sizeof(ntHeaders));
                        Logger::Info("[fix_imports] OEP set successfully");
                    }
                }
            }
        } catch (const std::exception& e) {
            Logger::Warning("[fix_imports] Failed to set OEP: {}", e.what());
        }
    }
    ...
    // Step 2: Fix imports via Scylla
    std::wstring wPath = ToFilesystemPath(filePath).wstring();
    auto fixResult = scylla.IatFixAuto(
        searchResult.iatStart, searchResult.iatSize,
        pid, wPath, wPath);

    if (!fixResult.success) {
        result.error = "Scylla IAT fix failed: " + fixResult.errorMessage;
        Logger::Error("[fix_imports] {}", result.error);
        return result;
    }
